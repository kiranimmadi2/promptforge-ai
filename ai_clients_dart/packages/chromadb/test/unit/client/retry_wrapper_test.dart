...
import 'package:chromadb/src/client/config.dart';
import 'package:chromadb/src/client/retry_wrapper.dart';
...
void main() {
  group('RetryWrapper', () {
    ...
    group('executeWithRetry', () {
      ...
      test('retries idempotent POST when isIdempotent=true', () async {
        var callCount = 0;
        final request = http.Request(
          'POST',
          Uri.parse('https://api.example.com'),
      ...
      test('retries on dart:async TimeoutException for GET', () async {
        var callCount = 0;
        final request = http.Request(
          'GET',
          Uri.parse('https://api.example.com'),
        ...
        final response = await wrapper.executeWithRetry(
          ...
          () async {
            callCount++;
            if (callCount < 2) {
              throw dart_async.TimeoutException('Request timed out');
            }
            return http.Response('{"ok": true}', 200);
          },
          null,
      ...
      test('does not retry dart:async TimeoutException for POST', () {
        var callCount = 0;
        final request = http.Request(
          'POST',
          Uri.parse('https://api.example.com'),
        ...
        expect(
          ...
          throwsA(isA<dart_async.TimeoutException>()),
        );

        expect(callCount, 1);
      });

      test('retries on package TimeoutException for GET', () async {
        var callCount = 0;
        final request = http.Request(
          'GET',
          Uri.parse('https://api.example.com'),
      ...
      test('does not catch AbortedException', () {
        ...
        expect(
          wrapper.executeWithRetry(
            ...
            () {
              callCount++;
              throw AbortedException(message: 'User cancelled');
            },
            null,
            'req_123',
          ),
      ...
      test('aborts during retry delay when trigger fires', () {
        ...
        final abortCompleter = dart_async.Completer<void>();
