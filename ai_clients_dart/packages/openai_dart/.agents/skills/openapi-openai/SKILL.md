...
```
3. Implement with `scaffold` plus the package references, then promote the reviewed candidate from `output_dir/latest-<spec>.json` into `packages/openai_dart/specs/` before final verification.
4. Verify:
```bash
