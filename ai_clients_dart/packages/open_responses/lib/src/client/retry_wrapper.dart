...
/// In the OpenResponses client integration, this wrapper is applied by the
/// interceptor chain only for regular `http.Request` instances. Multipart and
/// streamed requests are not retried to avoid issues with request body
/// re-consumption.
///
/// ## Retry Conditions
///
  ...
  Future<http.Response> executeWithRetry(
    ...
    while (attempt <= config.retryPolicy.maxRetries) {
      try {
        ...
        // Retry on package-specific timeout for idempotent methods only
        ...
        // Retry on dart:async timeout for idempotent methods only
        if (!_isIdempotent(request.method) ||
            attempt >= config.retryPolicy.maxRetries) {
          rethrow;
        }
